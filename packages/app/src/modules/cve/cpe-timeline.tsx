...
const SeverityDistributionPopup = dynamic(() => import("./pop-up"), {
  ssr: false,
});
...
const TimelineItem: React.FC<TimelineItemProps> = ({title, details, index}) => {
  ...
  const averageScore =
    details.reduce((acc, {baseScore}) => acc + baseScore, 0) / details.length;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography>{`Number of CVEs: ${
          averageScore <= 0 ? 0 : details.length
        }`}</Typography>
      </Paper>
      ...
      <SeverityDistributionPopup
        ...
        severities={details.map((detail) => ({
          value: detail.baseSeverity,
          count: 1,
        }))}
      />
    </Box>
      ...
      <Box
        sx={{
          ...
          "&:before, &:after": {
            ...
            width: "100%",
            background:
              "linear-gradient(180deg, #f7b267 0%, #f79d65 50%, #f4845f 100%)",
          },
          "&:before": {
